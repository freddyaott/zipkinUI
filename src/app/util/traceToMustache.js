...
import {Constants, ConstantNames} from './traceCostants';
...
function treeDepths(entry, startDepth) {
  ...
  return _(entry.children || []).reduce((prevMap, child) => {
    ...
    var newCombined = _extends({}, prevMap, childDepths);
    return newCombined;
  }, initial);
}
...
export function traceToMustache(trace, logsUrl = undefined) {
  ...
  const spans = _(getRootSpans(trace)).flatMap(
    ...
    var binaryAnnotations = (span.binaryAnnotations || []).map(function(a) {
      if (a.key === Constants.ERROR) {
        errorType = "critical";
      }
      var key = ConstantNames[a.key] || a.key;
    ...
    var localComponentAnnotation = _(span.binaryAnnotations || []).find(function(
      s
    ) {
      return s.key === Constants.LOCAL_COMPONENT;
    });
    if (localComponentAnnotation && localComponentAnnotation.endpoint) {
      binaryAnnotations.push(
        _extends2({}, localComponentAnnotation, {
          key: "Local Address",
